...
export function Navbar() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-[100] flex items-center justify-between border-b border-neutral-200 bg-white px-8 py-4 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
      <button
        type="button"
        className="font-mono text-xl font-medium tracking-wide text-neutral-900 transition-colors hover:text-blue-600 dark:text-neutral-100 dark:hover:text-blue-400"
      ...
      >
        {SECTIONS.map(({ id, label }) => (
          <button
            key={id}
            type="button"
            className={`border-0 bg-transparent p-0 font-sans text-[0.95rem] cursor-pointer transition-colors ${
              activeId === id
      ...
      <div className="flex items-center gap-4">
        <button
          type="button"
          className="relative h-6 w-11 rounded-xl border-0 bg-neutral-200 cursor-pointer dark:bg-neutral-700"
          onClick={toggleTheme}
        ...
        <button
          type="button"
          className="flex flex-col gap-1.5 border-0 bg-transparent p-2 cursor-pointer md:hidden"
